...
class ValueBasedLearner(ActorLearner):
    ...
    def __init__(self, args):
        ...
        if self.actor_id == 0:
            ...
            self.saver = tf.train.Saver(var_list=var_list, max_to_keep=3, keep_checkpoint_every_n_hours=2)
    ...
    def choose_next_action(self, state):
        ...
        network_output = self.session.run(self.local_network.output_layer, feed_dict={self.local_network.input_ph: [state]})[0]
...
class OneStepQLearner(ValueBasedLearner):
    ...
    def _run(self):
        ...
        logger.debug("Actor {} resuming at Step {}".format(self.actor_id, self.global_step.value()))
        ...
        while (self.global_step.value() < self.max_global_steps):
            ...
            if episode_over:
                ...
                q_target_values_new_state = self.session.run(self.target_network.output_layer, feed_dict={self.target_network.input_ph: [s_prime]})
