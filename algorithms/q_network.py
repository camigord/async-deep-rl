        ...
        with tf.name_scope(self.name):
            ...
            if "target" not in self.name: # Only for the training network
                ...
                if self.clip_loss_delta > 0:
                    quadratic_part = tf.minimum(tf.abs(diff), tf.constant(self.clip_loss_delta))
                    linear_part = tf.sub(tf.abs(diff), quadratic_part)

                    self.loss = tf.add(tf.nn.l2_loss(quadratic_part),tf.mul(tf.constant(self.clip_loss_delta), linear_part))
                else:
