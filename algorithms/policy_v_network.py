...
class PolicyVNetwork(Network):
    ...
    def __init__(self, conf):
        ...
        self.entropy_regularisation_strength = conf['args'].entropy_regularisation_strength
        ...
        with tf.name_scope(self.name):
            ...
            if recurrent_layer:
                ...
                with tf.variable_scope(self.name+'/'+layer_name) as vs:
                    ...
                    self.lstm_cell_state = tf.Variable(tf.zeros([1, self.lstm_cell.state_size]))
                    self.ox, self.lstm_cell_state = self.lstm_cell(self.o3, self.lstm_cell_state)
                    # Get all LSTM trainable params
                    self.lstm_trainable_variables = [v for v in tf.trainable_variables() if v.name.startswith(vs.name)]
            else:
                if self.arch == "NIPS":
                    self.ox = self.o3
                else: #NATURE
            ...
            self.wpi, self.bpi, self.output_layer_pi = self._softmax(layer_name, self.ox, self.num_actions)

            # Avoiding log(0) by adding a very small quantity (1e-30) to output.
            self.log_output_layer_pi = tf.log(tf.add(self.output_layer_pi, tf.constant(1e-30)), name= layer_name + '_log_policy')

            # Entropy: sum_a (-p_a ln p_a)
            self.output_layer_entropy = tf.reduce_sum(tf.mul(tf.constant(-1.0), tf.mul(self.output_layer_pi, self.log_output_layer_pi)), reduction_indices = 1)

            # Final critic layer
            self.wv, self.bv, self.output_layer_v = self._fc('fc_value4', self.ox, 1, activation = "linear")

            if self.arch == "NIPS":
                self.params = [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.wpi, self.bpi, self.wv, self.bv]
            else: #NATURE
                self.params = [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.w4, self.b4, self.wpi, self.bpi, self.wv, self.bv]
            ...
            log_output_selected_action = tf.reduce_sum(tf.mul(self.log_output_layer_pi, self.selected_action_ph), reduction_indices = 1)
            actor_objective_advantage_term = tf.mul(log_output_selected_action, self.adv_actor_ph)
            actor_objective_entropy_term = tf.mul(self.entropy_regularisation_strength, self.output_layer_entropy)
            self.actor_objective = tf.reduce_sum(tf.mul(tf.constant(-1.0), tf.add(actor_objective_advantage_term,actor_objective_entropy_term)))

            # Critic loss
            if self.clip_loss_delta > 0:
                quadratic_part = tf.minimum(tf.abs(self.adv_critic),tf.constant(self.clip_loss_delta))
                ...
                self.critic_loss = tf.mul(tf.constant(0.5), tf.nn.l2_loss(quadratic_part) + self.clip_loss_delta * linear_part)
            else:
                #OBS! For the standard L2 loss, we should multiply by 0.5. However, the authors of the paper
                # recommend multiplying the gradients of the V function by 0.5. Thus the 0.5
            ...
            if self.clip_norm_type == 'ignore':
                ...
                self.get_gradients = [tf.clip_by_norm(g, self.clip_norm) for g in grads]
            ...
            for p in self.params:
                self.params_ph.append(tf.placeholder(tf.float32,
                    shape=p.get_shape(),
                    name="shared_memory_for_{}".format((p.name.split("/", 1)[1]).replace(":", "_"))))

            # Ops to sync net with shared memory vars
            self.sync_with_shared_memory = []
            for i in xrange(len(self.params)):
                self.sync_with_shared_memory.append(self.params[i].assign(self.params_ph[i]))
