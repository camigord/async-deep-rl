...
class Network(object):
    ...
    def __init__(self, conf):
        ...
        with tf.name_scope(self.name):
            self.input_ph = tf.placeholder('float32',[None,84,84,4], name = 'input')
    ...
    def _conv2d(self, name, _input, filters, size, channels, stride, padding = 'VALID', init = "torch"):
        w = self._conv_weight_variable([size,size,channels,filters], name + '_weights', init = init)
        b = self._conv_bias_variable([filters], size, size, channels,name + '_biases', init = init)
    ...
    def _fc(self, name, _input, output_dim, activation = "relu", init = "torch"):
        ...
        w = self._fc_weight_variable([input_dim, output_dim], name + '_weights', init = init)
        b = self._fc_bias_variable([output_dim], input_dim, name + '_biases', init = init)
